...
def hackernews_topstory_ids() -> pd.DataFrame:
    """Get up to 500 top stores from the HackerNews topstories endpoint.
...
def hackernews_topstories(
    ...
    It may take 1-2 minutes to fetch all 500 items.
    ...
    for item_id in hackernews_topstory_ids["item_ids"]:
        ...
        results.append(item)
        if len(results) % 20 == 0:
            context.log.info(f"Got {len(results)} item so far.")
    ...
    # Dagster supports attaching arbitrary metadata to asset materalizations. This metadata will be 
    ...
    # Read more about the asset metadata in https://docs.dagster.io/concepts/metadata-tags/asset-metadata
...
def hackernews_topstoreis_word_cloud(
